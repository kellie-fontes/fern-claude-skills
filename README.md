...
Fern expects a specific folder layout. Before starting, make sure yours looks like this:

```
~/your-demo-folder/              ← run Fern skills from here (or from the sfdx-project subfolder)
├── fern-context.md              ← generated by /01-fern-design, lives here
├── reset-demo.sh                ← downloaded by install.sh
├── your-sfdx-project/           ← your Salesforce DX project (has sfdx-project.json)
│   └── force-app/
└── your-mule-project/           ← your MuleSoft app (has pom.xml)
```

Every Fern skill searches for `fern-context.md` in the current directory first, then one level up — so it works whether you run it from the parent folder or from inside the Salesforce DX project. **You never need to copy or move `fern-context.md`.**
